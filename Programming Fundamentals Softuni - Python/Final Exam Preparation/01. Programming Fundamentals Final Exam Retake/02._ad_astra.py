...
pattern = r"(?P<sep>[#|])(?P<type>[a-zA-z\s]+)(?P=sep)(?P<date>[0-9]{2}/[0-9]{2}/[0-9]{2})(?P=sep)(?P<cal>[0-9]+)(?P=sep)"
products = {
    "product": [],
    "dates": [],
    "calories": []
...
for i in range(len(items)):
    products["product"].append(items[i][1])
    products["dates"].append(items[i][2])
    products["calories"].append(items[i][3])

products["calories"] = [int(i) for i in products["calories"]]
days = sum(products["calories"]) // 2000
print(f"You have food to last you for: {days} days!")
for i in range(len(products["product"])):
    print(f"Item: {products['product'][i]}, Best before: {products['dates'][i]}, Nutrition: {products['calories'][i]}")
