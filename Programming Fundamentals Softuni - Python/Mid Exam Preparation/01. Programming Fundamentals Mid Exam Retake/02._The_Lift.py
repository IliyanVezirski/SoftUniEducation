...
people_lift = int(input())

lift = [int(i) for i in input().split()]
available_space = len(lift) * 4 - sum(lift)
if available_space >= people_lift:
...
if available_space >= people_lift:
    ...
    print(f"There isn't enough space! {abs(available_space - people_lift)} people in a queue!")
    print(f"{' '.join(lift)}")
